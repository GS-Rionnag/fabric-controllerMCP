...
| MCP implementation | Use the official MCP Java SDK 2.0.1 with Streamable HTTP. Embed Apache Tomcat 11.0.11 only as the minimal Servlet host; do not add Spring Boot. Bind the initial endpoint to `127.0.0.1:43125/mcp` and reject non-loopback requests and non-local browser origins. | 2026-09-13 |
...
| Raw client input | Provide a shared `input` command branch for raw keyboard and mouse events in both main-menu and in-game states. Dispatch through Minecraft's native input handlers, rather than the generic UI branch. Prefer direct semantic commands by default; camera control remains deferred. `ui.key_press` is intentionally absent. | 2026-09-14 |
